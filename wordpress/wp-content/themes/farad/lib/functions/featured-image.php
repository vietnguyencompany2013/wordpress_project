...
add_action( 'init', 'farad_add_image_sizes' );
function farad_add_image_sizes() {
	add_image_size( 'farad-image-featured', 310, 210, true );
}

/** Farad Get Image Id Manual */
function farad_get_image_id_manual( $num = 0 ) {
...
/** Farad Get Image Id Auto */
function farad_get_image_id_auto( $num = 0 ) {
	
	/** Global Object */
	global $post;
	
	/** Manual Check At Priority */
	$id = farad_get_image_id_manual( $num );
...
/** Farad Get Image*/
function farad_get_image( $args = array() ) {
	...
	$id = '';
	if( $args['mode'] == 'manual' ) {		
		$id = farad_get_image_id_manual( $args['num'] );			
	} else {	
		$id = farad_get_image_id_auto( $args['num'] );	
	}
